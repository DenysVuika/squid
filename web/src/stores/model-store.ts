    ...
    (set, get) => ({
      ...
      loadModels: async () => {
        ...
        try {
          ...
          const [{ models: fetchedModels }, config] = await Promise.all([fetchModels(''), fetchConfig('')]);
          ...
          if (fetchedModels.length > 0) {
            ...
            const isCurrentModelValid =
              currentSelectedModel && fetchedModels.some((m) => m.id === currentSelectedModel);
          ...
          toast.error('Failed to connect to model provider', {
            description: 'Could not reach the models API. Check if backend is running.',
            duration: 5000,
          });
          set({ isLoading: false });
        }
